...
const StoryReader = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      axios
        .post("http://localhost/Website-Truyen/Api/Chapter/IncreaseView.php", {
          chapterID: ChapterID,
        })
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost/Website-Truyen/Api/Chapter/Chapter.php?ChapterID=${ChapterID}`
        );
  ...
  if (!data)
    return <p className="text-white text-center mt-10">Đang tải chương...</p>;
  ...
  return (
    <div>
      <div className="bg-[#111] min-h-screen text-white flex flex-col items-center">
        ...
        <div className="flex items-center justify-center mt-4 gap-3 text-gray-300">
          <button
            ...
            onClick={() =>
              data.PrevChapterID &&
              navigate(
                `/read/${data.PrevChapterID}?story=${StoryID}&storyname=${StoryName}`
              )
            }
          >
            &lt; Trước
          ...
          <button
            ...
            onClick={() =>
              data.NextChapterID &&
              navigate(
                `/read/${data.NextChapterID}?story=${StoryID}&storyname=${StoryName}`
              )
            }
          >
            Sau &gt;
        ...
        <h1 className="flex flex-col items-center mt-6 gap-4 w-full text-4xl font-bold">
          {StoryName}
        </h1>

        {/* Nội dung chương - chỉnh để dễ đọc: chữ lớn hơn, căn đều hai bên, khoảng cách dòng tốt */}
        <div className="flex justify-center mt-6 w-full px-4">
          {/* wrapper với nền nhạt hơn nền chính */}
          <div className="w-full max-w-3xl bg-[rgba(255,255,255,0.03)] rounded-lg p-8">
            <article className="text-lg md:text-xl leading-8 md:leading-9 text-justify text-gray-100 whitespace-pre-wrap">
              {data.Content}
            </article>
          </div>
        </div>
