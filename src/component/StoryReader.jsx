...
const StoryReader = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      axios.post("http://localhost/Website-Truyen/Api/Chapter/IncreaseView.php", {
      chapterID: ChapterID
      })
  ...
  useEffect(()=>{
    const timer = setTimeout(() => {
      axios.post("http://localhost/Website-Truyen/Api/Chapter/AddHistory.php", {
      chapterID: ChapterID,
      storyID: StoryID,
      userID: user?.UserID
      })
  ...
  useEffect(()=>{
const timer = setTimeout(() => {
      axios.post("http://localhost/Website-Truyen/Api/UpdateHistory.php", {
      chapterID: ChapterID,
      storyID: StoryID,
      userID: user?.UserID
      })
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {        
      try {
        ...
        .get(`http://localhost/Website-Truyen/Api/Chapter/Chapter.php?ChapterID=${ChapterID}`);
  ...
  return (
    <div>
      <div className="bg-[#111] min-h-screen text-white flex flex-col items-center">
        ...
        <h1 className="flex flex-col items-center mt-6 gap-4 w-full text-4xl font-bold">{StoryName}</h1>
        {/* noi dung */}
        <div className="flex flex-col items-center mt-6 gap-4 w-full whitespace-pre-wrap">
          {data.Content}
        </div>
